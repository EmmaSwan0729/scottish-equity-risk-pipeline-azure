...
def test_upload_df_to_adls_builds_correct_path():
    ...
    with patch("upload_to_adls.get_service_client"):
        result = upload_df_to_adls(
            df=test_df, account_name="teststorage", container="raw", date="2026-07-15"
        )
...
def test_upload_df_to_adls_calls_get_file_client_with_correct_path():
    ...
    mock_get_client.return_value.get_file_system_client.return_value.get_file_client.assert_called_with(
        "stock_prices/dt=2026-07-15/stock_prices.parquet"
    )
